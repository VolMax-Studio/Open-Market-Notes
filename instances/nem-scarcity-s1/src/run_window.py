...
def get_window_utc_bounds(window_str, n_baseline_months=12):
...
def execute_window_run(instance_dir, target_window):
    ...
    comp_zones = params.get('comparison_zones', ["NSW1", "QLD1", "SA1", "VIC1"])
    companion_zones = params.get('companion_zones', ["TAS1"])
    ...
    for z in comp_zones + companion_zones:
        ...
        filepath = os.path.join(inputs_dir, f'nem_{z}.feather')
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"Input file missing: {filepath}")

        col_name = params.get('series_bindings', {}).get(z, {}).get('baseline_col', 'RRP')
        interval_sec = float(params.get('series_bindings', {}).get(z, {}).get('interval_duration_sec', 300.0))
    ...
    result_data = {
        "instance_id": params.get('instance_id', 'nem-scarcity-s1'),
    ...
    print(f"[{target_window}] Status: {evaluation_status} | Label: {label} | Elevated Zones: {n_elevated_v073}/{len(comp_zones)} | result.json SHA-256: {result_sha256[:12]}...")

    return result_data, result_sha256

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run NEM Scheduled Window Evaluation")
    parser.add_argument('--window', type=str, required=True, help='Target window in YYYY-MM format')
    parser.add_argument('--instance', type=str, default='/home/volmax-studio/volmax-projects/iot2/PORTFOLIO/Open-Market-Notes/instances/nem-scarcity-s1', help='Instance directory')
