...
def run_multiclass_falsification_suite():
    ...
    test_cases.append(("A0: Envelope Digest Tampering", t_a0, "GATE_FAIL_A: INTEGRITY_DIGEST_MISMATCH", inputs_dir, script_path, False))

    # A1: Claim Window Mutation
    t_a1 = copy.deepcopy(base_v)
    t_a1["claim"]["window"] = "2026-08"
    resign(t_a1)
    test_cases.append(("A1: Claim Window Mutation", t_a1, "GATE_FAIL_A: CLAIM_EVIDENCE_MISMATCH", inputs_dir, script_path, False))

    # A2: Evidence Boundary Interval Tampering
    t_a2 = copy.deepcopy(base_v)
    t_a2["evidence_boundary"]["received_intervals"] = 8900
    resign(t_a2)
    test_cases.append(("A2: Evidence Boundary Forgery", t_a2, "GATE_FAIL_A: EVIDENCE_INTEGRITY_MISMATCH", inputs_dir, script_path, False))

    # A3: Frozen Rule Parameter Drift
    t_a3 = copy.deepcopy(base_v)
    t_a3["frozen_rule"]["parameters"]["s_thresh_pct"] = 5.0
    resign(t_a3)
    test_cases.append(("A3: Post-Hoc Threshold Drift", t_a3, "GATE_FAIL_A: FROZEN_RULE_HASH_MISMATCH", inputs_dir, script_path, False))

    # A4: Verdict Inversion
    t_a4 = copy.deepcopy(base_v)
    t_a4["verdict"]["label"] = "ELEVATED"
    resign(t_a4)
    test_cases.append(("A4: Verdict Inversion Bias", t_a4, "GATE_FAIL_A: DETERMINISTIC_LOGIC_VIOLATION", inputs_dir, script_path, False))

    # A5: Admissibility Policy Spoofing
    t_a5 = copy.deepcopy(base_v)
    t_a5["admissibility"]["source_license"] = "PROPRIETARY_NDA_CLOSED"
    resign(t_a5)
    test_cases.append(("A5: Admissibility License Spoof", t_a5, "GATE_FAIL_A: ADMISSIBILITY_POLICY_VIOLATION", inputs_dir, script_path, False))

    # =========================================================================
    # KLASA B TESTS: Evidence Binding & Telemetry Corruption
    # =========================================================================
    # B1: Physical Telemetry Bit-Flip on Disk (modify copy of telemetry store)
    # We will test B1 dynamically in temp evidence store
    test_cases.append(("B1: Telemetry Data Corruption on Disk", "DYNAMIC_B1", "GATE_FAIL_B: TELEMETRY_CORRUPTION", "DYNAMIC_STORE", script_path, False))
    ...
    resign(t_b2)
    test_cases.append(("B2: Manifest Hash Forgery", t_b2, "GATE_FAIL_B: INPUT_MANIFEST_HASH_MISMATCH", inputs_dir, script_path, False))
    ...
    resign(t_b3)
    test_cases.append(("B3: Frozen Script Hash Mismatch", t_b3, "GATE_FAIL_B: FROZEN_RULE_HASH_MISMATCH", inputs_dir, script_path, False))
    ...
    resign(t_c1)
    test_cases.append(("C1: Broken Reproduce Recipe", t_c1, "GATE_FAIL_C: REPRODUCTION_FAILED", inputs_dir, script_path, True))

    # C2: Output Agreement Discrepancy (Declared metric changed from 4.8360 to 9.9999, re-signed, passes Class A and B, but fails Class C re-execution!)
    t_c2 = copy.deepcopy(base_v)
    t_c2["execution_and_metrics"]["zone_metrics"]["NSW1"]["m1_pct"] = 9.9999
    resign(t_c2)
    test_cases.append(("C2: Output Agreement Discrepancy (Fails Cold Re-Execution)", t_c2, "GATE_FAIL_C: OUTPUT_AGREEMENT_MISMATCH", inputs_dir, script_path, True))
    ...
    try:
        for name, v_data, expected_err, in_d, sc_p, do_cold in test_cases:
            target_v_file = os.path.join(temp_root, "test_verdict.json")
            target_in_d = in_d

            # Handle dynamic B1 test
            if v_data == "DYNAMIC_B1":
                # Create a cloned inputs dir with corrupted NSW1 file
                b1_dir = os.path.join(temp_root, "corrupted_inputs")
                shutil.copytree(inputs_dir, b1_dir)
                # Corrupt one byte in nem_NSW1.feather
            ...
            try:
                verify_gate(target_v_file, target_in_d, sc_p, run_cold_reexecution=do_cold)
