...
def get_window_utc_bounds(window_str):
    # window_str format: YYYY-MM
    ...
    # Rolling 12M baseline start: 1 year prior to p_start
    b_start_year = year - 1
    b_start_month = month
    b_start = f"{b_start_year:04d}-{b_start_month:02d}-01T00:00:00Z"
    
    # Baseline end: day before p_start
    b_end_year = year if month > 1 else year - 1
    b_end_month = month - 1 if month > 1 else 12
    b_end_last_day = calendar.monthrange(b_end_year, b_end_month)[1]
    b_end = f"{b_end_year:04d}-{b_end_month:02d}-{b_end_last_day:02d}T23:59:59Z"
...
def execute_window_run(instance_dir, target_window):
    ...
    p_start, p_end, b_start, b_end, days_in_month = get_window_utc_bounds(target_window)
    ...
    print(f"\n=======================================================")
    print(f"      ENTSOE-SCARCITY-S1 RUN EXECUTED ({target_window})")
    print(f"=======================================================")
    print(f"Window: {target_window}")
    print(f"Evaluation Status: {evaluation_status}")
    print(f"Label: {label}")
    print(f"Result JSON SHA-256: {result_sha256}")
    print(f"Series Log: {log_path}\n")

    return result_data, result_sha256

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run S1 Scheduled Window Evaluation")
    parser.add_argument('--window', type=str, required=True, help='Target window in YYYY-MM format')
    parser.add_argument('--instance', type=str, default='/home/volmax-studio/volmax-projects/iot2/PORTFOLIO/Open-Market-Notes/instances/entsoe-scarcity-s1', help='Instance directory')
