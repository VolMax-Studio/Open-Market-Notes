# PROVENANCE — Instance entsoe-scarcity-s1

> **Instance ID:** `entsoe-scarcity-s1`
> **Selection Mode:** `S1_scheduled` (Scheduled Cadence Operation)
...
Telemetry files in `inputs/` are extended forward per `SERIES_TEMPLATE.md` §3. Manifest records are append-only.
...
- Absolute zero relative paths (`../`) escaping `instances/entsoe-scarcity-s1/` in execution code.
