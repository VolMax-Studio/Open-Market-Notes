...
Repository: VolMax-Studio/Open-Market-Notes (instances/entsoe-scarcity-s2)
Branch: instances/entsoe-scarcity-s2
Base scaffold commit: 378cb05
...
Candidate (SPREMNO ZA PRE-EXECUTION GEJT)
...
Public artifacts:
- ENTSO-E Transparency Platform API (DocumentType A85 / processType A16 / IEC 62325 XML).
- Raw HTTP response bodies and XML market documents captured at T_vintage within the official acquisition batch.
- Pinned EIC codes: 10YAT-APG------L (AT), 10YBE----------X (BE), 10YDK-1--------W (DK_1), 10YDK-2--------T (DK_2), 10YFR-RTE------C (FR), 10YNL----------L (NL).

Hashes / DOI / commit SHA:
Raw response hashes captured at execution post-freeze under PREREG_SHA.

Missing artifacts:
Enumerate explicitly at L0 / during acquisition; zero silent imputation.

Evidence owner:
ENTSO-E (source publisher) / TSOs (originating telemetry).

Independence class:
Self-reproduction audit (claimant = VolMax Studio). Note: Evaluated against external primary public API.
...
PT15M resolution. Baseline: 32,064 expected intervals per zone (2025-08-01 00:00 to 2026-07-01 00:00 local, converted to UTC). July 2026 Target: 2,976 expected intervals per zone (2026-07-01 00:00 to 2026-08-01 00:00 local, converted to UTC).
...
Gate outcome: SPREMNO ZA PRE-EXECUTION GEJT
Blocking findings:
Draft specification on branch spec-draft pending review.
