...
- **Governance Mandate:** `PARAMS.md` remains strictly byte-identical to the published Zenodo record (`9efbc2ec7d69c76d4a70070bb3c0b00b7528d56c90b314f4f8444ef581a0ed09`). Rather than modifying the frozen parameter document retroactively, five deviations between `PARAMS.md` (frozen at `6df5bcc`) and the execution code that produced `c192e7ee...` (at commit `29012c8`) are recorded here:
  ...
  4. **BESS Denominator Capacities & Attribution:** `PARAMS.md` at `6df5bcc` contained no explicit BESS nameplate capacity table. Energy denominators reside in `reproduce.py` (`BESS_ENERGY_CAPACITY`) and were verified against the official AEMO NEM Registration and Exemption List (`NEM-Registration-and-Exemption-List.xls`, published 15 May 2026, sheet "Generators and Scheduled Loads", accessed 2026-07-30 via AEMO Registration Portal: `https://www.aemo.com.au/energy-systems/electricity/national-electricity-market-nem/participate-in-the-market/registration`; e.g. Rangebank BESS `RANGEB1` = 400.0 MWh / 200.0 MW registered power capacity $\times$ 2.0 hour registered energy duration), as recorded in Entry #000.
