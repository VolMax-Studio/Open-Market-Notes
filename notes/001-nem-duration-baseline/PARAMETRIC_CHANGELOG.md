...
- **Governance Mandate:** `PARAMS.md` remains strictly byte-identical to the published Zenodo record (`9efbc2ec7d69c76d4a70070bb3c0b00b7528d56c90b314f4f8444ef581a0ed09`). Rather than modifying the frozen parameter document retroactively, five deviations between `PARAMS.md` (frozen at `6df5bcc`) and the execution code that produced `c192e7ee...` (at commit `29012c8`) are recorded here:
  1. **Separation Rule (Metric 1):** `PARAMS.md` states: *"Events separated by <30 minutes (less than 6 intervals)... are counted as separate events (no merging/smoothing)."* The frozen text is under-specified (it defines behavior for gaps under 30 minutes but remains silent on longer breaks). The producing code (`reproduce.py`) evaluates continuous sequences of $\ge \$300/\text{MWh}$ intervals and splits on any single interval below $\$300/\text{MWh}$ without multi-interval merging (as correctly documented in Note README Section 2).
  ...
  4. **BESS Denominator Capacities & Attribution:** `PARAMS.md` at `6df5bcc` contained no explicit BESS nameplate capacity table. Energy denominators reside in `reproduce.py` (`BESS_ENERGY_CAPACITY`) and were calibrated prior to publication against the official AEMO NEM Registration and Exemption List release (`NEM-Registration-and-Exemption-List.xls`, published 15 May 2026, sheet "Generators and Scheduled Loads"; e.g. Rangebank BESS `RANGEB1` = 400.0 MWh / 200.0 MW registered power capacity $\times$ 2.0 hour registered energy duration), as recorded in Entry #000. Live external portal URLs are ephemeral and subject to CDN redirects; the 15 May 2026 registration release governs the baseline denominators.
