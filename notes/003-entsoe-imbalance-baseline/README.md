# Open Market Note #003 — ENTSO-E Imbalance Price Duration Baseline
...
> **Data Provenance & License:** Primary ENTSO-E Transparency Platform (Imbalance Prices [17.1.g / 17.2.f]). Formally listed under CC BY 4.0 free re-use (Item #27). All raw files anchored with SHA-256 hashes in [`data_manifest.json`](./data_manifest.json).
...
To execute the complete analysis pipeline and regenerate `summary.json` and `results.json`:
```bash
python3 run_imbalance_analysis.py
```
