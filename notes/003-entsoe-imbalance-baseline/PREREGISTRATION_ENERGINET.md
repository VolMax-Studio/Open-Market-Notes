...
The 44 timestamps of the synchronous 2025-08-10 event, each checked in both
price areas: 88 lookups total.

The 9 asynchronous gap events from Note #003 are OUT OF SCOPE and are not
checked, reported, or referenced in the verdict.
...
Aggregate reported as counts out of 88, per category. No single verdict is
issued for the set.

## 5. Frozen non-goals

Not tested: correctness of either price; reason for any value difference;
quality of either source; cause of the outage; economic significance; the
9 asynchronous events.

If a result raises any of these, it becomes a separate artefact.

## 6. Termination

The check ends when all 88 lookups carry a verdict, or when the identity in
