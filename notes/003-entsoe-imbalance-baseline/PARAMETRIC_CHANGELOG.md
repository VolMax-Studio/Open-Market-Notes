...
- **Audit & Governance Findings:**
  ...
  2. **Three-Axis Taxonomy Verification & Live Ingestion Scope:**
  ...
  3. **Empirical Resolution of Danish Gap Count (55 $\rightarrow$ 53) & Disambiguation of Events vs MTUs:**
     - Verification of the full 13-month dataset confirms that `2026-05-31 22:00:00 UTC` is present in both `DK_1` (108.32 €/MWh) and `DK_2` (151.02 €/MWh). The resolution of this month-boundary acquisition artifact reduced the total Danish discrete gap event count from 55 to **53** (26 in `DK_1`, 27 in `DK_2`), accounting for **99 missing MTU intervals** (49 in `DK_1`, 50 in `DK_2`).
     - **Synchronous Telemetry Dropout Measurement (10 August 2025):** Mathematical set evaluation on 10-11 August 2025 reveals that $\text{set}(dk1\_gap\_starts) \oplus \text{set}(dk2\_gap\_starts) = \emptyset$ (empty symmetric difference) and zero duration mismatches. Exactly **22 discrete gap events per zone (44 of 53 total recorded events, 83.0%)** comprising **44 missing MTUs per zone (88 of 99 total missing MTUs, 88.9%)** occurred synchronously on the ENTSO-E Transparency Platform across both Danish bidding zones on 10 August 2025.
     - **Epistemic Classification:** The synchronous data gap on the ENTSO-E Transparency Platform is recorded as an empirical measurement [L1 Descriptive]. The underlying root cause (TSO grid infrastructure vs Transparency Platform delivery/dispatch layer) remains unverified without primary unavailability notices [L2 Hypothesis].
  4. **Epistemic Status of Danish Baseline Metrics under Post-Hoc Parameter:**
     - Because `max_telemetry_gap_minutes: 90` is a post-hoc empirical calibration fit to historical Danish data with 0-minute margin ($90\text{m observed} \le 90\text{m limit}$), any baseline metric or claim for `DK_1`/`DK_2` evaluated under this quality threshold is classified as:  
       `Measurement: L1 / Pre-registration: Exploratory / Verdict: None (Cannot carry a confirmatory verdict)`.
     - Central European zones (`NL`, `BE`, `FR`, `AT`) achieve 37,920/37,920 nominal MTUs (0 gaps) and retain full Confirmatory (L1) status.
  5. **Operational Verification Status & Untested Ingestion Paths:**
     - The monthly chunking algorithm and junction verifier are empirically validated over the 13-month historical window.
     - **Explicitly Untested Code Paths:** (a) The `manifest_verified_cache` code path (~line 394), and (b) the complete end-to-end live CLI wrapper (`download_entsoe_imbalance`) across all 6 zones in a single execution remain to be run upon live production generation.
- **Status:** Ratified (Merged to main)
