...
- **Dual Cause & Empirical Audit Findings:**
  ...
  2. **Dual Cause Breakdown & Verbatim Audit Findings:**
     - **Data Acquisition Artifact (5 breaches):** 5 of 8 total gap breaches occurred simultaneously across AT, BE, DK_1, DK_2, and NL at `2026-05-31 21:45 UTC` $\rightarrow$ `2026-05-31 22:15 UTC` at the boundary of monthly ENTSO-E CSV chunk stitching in `download_entsoe_data.py`. This chunk boundary is the only month-end boundary in the 13-month dataset where an active scarcity event ($\ge €100/\text{MWh}$) was ongoing across the boundary.
     - **Raw TSO Telemetry Gaps (3 breaches):** 3 gap breaches occurred in Denmark: 1 breach in DK_1 (`2025-08-10 18:45 UTC` $\rightarrow$ `19:15 UTC`) and 2 separate breaches in DK_2 (`2025-08-10 18:45 UTC` $\rightarrow$ `19:15 UTC` and `19:45 UTC` $\rightarrow$ `20:15 UTC`) representing actual Energinet TSO telemetry gaps. In DK_2, both 30-minute gaps occurred within a single ongoing extended scarcity event, producing 3 gap breaches across 2 bridged events.
