...
> **Version:** 3.0.0-draft (Strict Gap-Terminated & Parametrically Audited Spec)  
...
- **Dual Cause & Verbatim Empirical Audit Findings:**
  ...
  - **Dual Cause Breakdown:**
    - **Pan-European ENTSO-E Platform Publication Outage (5 breaches across 5 zones):** All 6 zones (AT, BE, DK_1, DK_2, FR, NL) possess an identical 30-minute timestamp gap at `2026-05-31 21:45 UTC` $\rightarrow$ `22:15 UTC` (missing 22:00 UTC Market Time Unit). `download_entsoe_data.py` executes single-call window requests (zero monthly loop stitching). This gap represents a coordinated pan-European publication outage on the ENTSO-E platform. In FR, price drops to €96.3/MWh after the gap, so no breach occurs; in AT, BE, DK_1, DK_2, and NL, prices remain $\ge €100/\text{MWh}$ across the gap, generating 5 gap breaches across 5 zones.
    - **Raw Energinet TSO Telemetry Gaps (3 breaches across 2 zones):** Out of 55 total telemetry gaps in Denmark across the 13-month baseline (27 in DK_1, 28 in DK_2), exactly 3 gap breaches fall within $\ge €100/\text{MWh}$ scarcity events: 1 breach in DK_1 (`2025-08-10 18:45 UTC` $\rightarrow$ `19:15 UTC`) and 2 separate breaches in DK_2 (`2025-08-10 18:45 UTC` $\rightarrow$ `19:15 UTC` and `19:45 UTC` $\rightarrow$ `20:15 UTC`). In DK_2, both 10 August 2025 gaps occurred within a single ongoing extended scarcity event (Bridged Event #1), while the 31 May 2026 gap occurred in a separate event 10 months later (Bridged Event #2), totaling 3 gap breaches across 2 bridged events in DK_2.
