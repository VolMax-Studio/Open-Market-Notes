...
def verify_crypto_signature(commit_sha: str, allowed_signers_path: str, keys_asc_path: str) -> bool:
    """
    Verifies that commit_sha carries a valid cryptographic signature specifically
...
def verify_repo_protected_paths(base_ref: str = "origin/main") -> int:
    """
    L-1 Fix (§7.1): Inspects all commits modifying protected governance paths:
      - .github/workflows/**
      - governance/HUMAN_KEYS/**
      - scripts/p10_state.py
    Ensures every such commit is cryptographically signed by a key present in the parent registry.
    """
    log_diag(f"Checking protected paths across git range {base_ref}..HEAD...")
    try:
        ...
        # Fallback if origin/main not fetched
        commits = subprocess.check_output(
    ...
    protected_patterns = [
        ...
        re.compile(r"^governance/HUMAN_KEYS/"),
        re.compile(r"^scripts/p10_state\.py$")
    ]
    ...
    for c in commits:
        ...
        touches_protected = any(any(pat.search(f.strip()) for pat in protected_patterns) for f in files if f.strip())
        if touches_protected:
            log_diag(f"Commit {c[:10]} touches protected governance/CI paths: {files}")
            # Verify signature against parent commit's HUMAN_KEYS registry
            with tempfile.TemporaryDirectory() as tmpdir:
                parent_allowed = os.path.join(tmpdir, "allowed_signers")
                parent_keys = os.path.join(tmpdir, "keys.asc")
                has_parent_reg = False

                try:
                    p_allowed_content = subprocess.check_output(
                        ["git", "show", f"{c}~1:governance/HUMAN_KEYS/allowed_signers"],
                ...
                try:
                    p_keys_content = subprocess.check_output(
                        ["git", "show", f"{c}~1:governance/HUMAN_KEYS/keys.asc"],
                        cwd=REPO_ROOT, stderr=subprocess.DEVNULL
                    )
                ...
                # If no parent registry exists (bootstrap commit), check current tree registry
                if not has_parent_reg:
                    cur_allowed = os.path.join(REPO_ROOT, "governance", "HUMAN_KEYS", "allowed_signers")
                    cur_keys = os.path.join(REPO_ROOT, "governance", "HUMAN_KEYS", "keys.asc")
                    sig_ok = verify_crypto_signature(
                        c,
                        cur_allowed if os.path.exists(cur_allowed) else None,
                        cur_keys if os.path.exists(cur_keys) else None
                    )
                else:
                    sig_ok = verify_crypto_signature(
                        c,
                ...
                if not sig_ok:
                    violations.append(f"Commit {c[:10]} touches protected path but lacks verified cryptographic signature from HUMAN_KEYS")

    if violations:
        for v in violations:
            log_diag(f"  [X] {v}")
        return EXIT_BLOCKED
    log_diag("Protected paths verification [PASS]")
    return EXIT_PASS
...
class P10StateGuard:
    ...
    def _eval_human_phase(self, phase: str, pinfo: dict) -> bool:
        ...
        if not verify_crypto_signature(commit_sha, allowed_signers, keys_asc):
        ...
        # L-2 Fix: Strict granular token check — exactly 1 signoff token matching this phase
        try:
            commit_msg = subprocess.check_output(
                ["git", "log", "-n", "1", "--format=%B", commit_sha],
                cwd=REPO_ROOT, text=True
            )
            all_signoff_tokens = re.findall(r"P10-SIGNOFF:\s*([A-Za-z0-9_]+)", commit_msg)
            if not all_signoff_tokens or phase not in all_signoff_tokens:
                self.block(phase, f"Commit message for {commit_sha} missing 'P10-SIGNOFF: {phase}' token")
                return False
            if len(all_signoff_tokens) > 1:
                self.block(phase, f"Granularity violation in {commit_sha}: found multiple signoff tokens ({all_signoff_tokens}); each HUMAN phase requires an individual dedicated signed commit")
                return False
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--instance", default=None, help="Path to instance directory (e.g. instances/fr-be-de-eclipse-coupling-probe)")
    parser.add_argument("--verify-repo", action="store_true", help="Verify protected governance and CI paths (L-1)")
    parser.add_argument("--base-ref", default="origin/main", help="Base git reference for repo protection range (default: origin/main)")
    ...
    if args.verify_repo:
        repo_res = verify_repo_protected_paths(args.base_ref)
        if repo_res != EXIT_PASS:
            overall_exit = repo_res

    if args.instance:
        guard = P10StateGuard(args.instance, github_token=args.github_token, report_only=args.report_only)
        inst_res = guard.run()
        if inst_res != EXIT_PASS:
            overall_exit = inst_res

    if not args.verify_repo and not args.instance:
        parser.print_help()
        sys.exit(EXIT_ERROR)

    sys.exit(overall_exit if not args.report_only else EXIT_PASS)
